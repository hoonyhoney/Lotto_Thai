package com.example.lotto_thai
...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        resultLabel.text = "랜덤으로 생성된 Wn로또번호 입니다"

        if(!TextUtils.isEmpty(name)){
            resultLabel.text = "${name} 님의 Wn${SimpleDateFormat("yyyy년 MM월 dd일").format(Date())}Wn로또 번호입니다"
        }

        if(!TextUtils.isEmpty(constellation)){
            resultLabel.text = "${constellation} 의 Wn${SimpleDateFormat("yyyy년 MM월 dd일").format(Date())}Wn로또 번호입니다"
        }
